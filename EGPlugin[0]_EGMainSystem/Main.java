...
public class Main extends JavaPlugin implements Listener
{
	...
	public void onEnable()
	{
		...
		blueFlag = new ItemStack(35, 1, (byte) 14);
	...
	public static void GameJoin(Player p)
	{
		...
		else
		{
			...
			{
				Startgame();
				Bluecore_loc.getBlock().setType(Material.EMERALD_BLOCK);
				Redcore_loc.getBlock().setType(Material.EMERALD_BLOCK);
			}
			for(Player t : plist){
	...
	public static void Startgame()
	{
		...
		tasktime[cur] = 5;
		Bukkit.broadcastMessage(MS+"CTF 게임이 곧 시작됩니다.");
		...
		if(e.getAction() == Action.RIGHT_CLICK_BLOCK || e.getAction() == Action.LEFT_CLICK_BLOCK){
			...
			if(l.equals(RedArrive_loc) && redlist.contains(p) && !game_end){
				if(p.getInventory().contains(35)){
					...
					Sendmessage(MS+"§c§l레드팀 §a§l"+p.getName()+"§6§l 님이 깃발을 전달했습니다!");
					Redcore_cnt++;
	...
	public void onDamage(EntityDamageByEntityEvent e)
	{
		if(e.getEntity() instanceof Player)
		{
			...
			if(d == null || !(plist.contains(d) && jlist.containsKey(p.getName())))
			{
				return;
			}
